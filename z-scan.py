...
def browse_button():
    ...
    parameter_var.set(export.text.parameter_string(PARAMETER_DATA))
    result_var.set('Run to generate results')
    parameter_frame.update_idletasks()
    
    # Plot raw data
    fig1 = export.plot.raw(MEASUREMENT)
...
def run():
    ...
    BOUNDS = zscan.general.bounds_convert(bounds)
    

    ### Fit type
    FIT_TYPE = zscan.general.fit_type_convert(fit_type)
    ...
    # Run model
    RUNS, P_BEST, CHI2_BEST = zscan.run(MEASUREMENT, FIT_TYPE, N_RUNS, P0, MODEL_PARAMETERS, EXPERIMENT_PARAM, 
                                                pb_frame, iter_count_label, value_label, age_count_label, pb)
    SIGMA_P, CHI2_SPAN = zscan.errorbar(MEASUREMENT, FIT_TYPE, P_BEST, CHI2_BEST, EXPERIMENT_PARAM)
    OUTPUT_STRING = export.text.output(FIT_TYPE, CHI2_BEST, P_BEST, SIGMA_P, CHI2_SPAN)
    ...
    # Plots
    TITLE, fig2, fig2_1, fig2_2, fig3 = zscan.general.generate_plots(
...
iter_label.grid(row=3, column=0, pady=5, sticky=tk.E)
iter_content.grid(row=3, column=1, pady=5, sticky=tk.W)
#### MAX AGE
...
age_label.grid(row=4, column=0, pady=5, sticky=tk.E)
age_content.grid(row=4, column=1, pady=5, sticky=tk.W)
#### T
t = tk.StringVar(model_frame, '0.8')
t_label = ttk.Label(model_frame, text='T = ')
t_content = tk.Entry(model_frame, textvariable=t)
t_label.grid(row=5, column=0, pady=5, sticky=tk.E)
t_content.grid(row=5, column=1, pady=5, sticky=tk.W)
#### MAX JUMP
...
jump_label.grid(row=6, column=0, pady=5, sticky=tk.E)
jump_content.grid(row=6, column=1, pady=5, sticky=tk.W)
#### MAX REJECT
...
reject_label.grid(row=7, column=0, pady=5, sticky=tk.E)
reject_content.grid(row=7, column=1, pady=5, sticky=tk.W)
...
parameter_var = tk.StringVar(parameter_frame, export.text.parameter_string(PARAMETER_DATA))
...
export_button = ttk.Button(window, text='Export', command=lambda: zscan.general.export(
