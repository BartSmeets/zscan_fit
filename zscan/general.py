...
import zscan.export
from zscan.absorption import intensity
...
def generate_plots(MEASUREMENT, FILE_NAME, FIT_TYPE, N_RUNS, P_BEST, EXPERIMENT_PARAM, RUNS):
    ...
    zscan.export.plot.Tz(ax2, Z_DATA, I_DATA, Z_PLOT, SIGMA_PLOT, FIT_TYPE, P_BEST, EXPERIMENT_PARAM)
    ## Labels
    ax2.set_xlabel('z [mm]')
    ...
    ax2_3 = ax2_2.twinx()  
    zscan.export.plot.Tz(ax2_2, Z_DATA, I_DATA, Z_PLOT, SIGMA_PLOT, FIT_TYPE, P_BEST, EXPERIMENT_PARAM)
    ...
    for i in range(N_RUNS):
        textstr = zscan.export.text.individual(FIT_TYPE, RUNS, i)
        zscan.export.plot.Tz(ax3[i], Z_DATA, I_DATA, Z_PLOT, SIGMA_PLOT, FIT_TYPE, RUNS[i,:-1], EXPERIMENT_PARAM)
...
def export(DIRECTORY, TITLE, OUTPUT_STRING, fig1, fig2, fig2_1, fig2_2, fig3, PARAMETER_DATA):
    ...
    output_file.write(zscan.export.text.parameter_string(PARAMETER_DATA))
    output_file.write('\n\n')
